...
echo ""
echo "================================="
echo "  Building datas for $ROUTER_NAME"
echo "================================="
echo ""

echo Downloading osm data
curl -s http://dev.hsl.fi/osm.finland/finland.osm.pbf -o $ROUTER_FINLAND/finland.osm.pbf >/dev/null &
curl -s http://dev.hsl.fi/osm.hsl/hsl.osm.pbf -o $ROUTER_HSL/hsl.osm.pbf > /dev/null &
wait
...
if [ "$ROUTER_NAME" == "hsl" ]; then
    retrieveHsl
    cd $ROOT
    zip -D ${DEST}/router-hsl.zip router-hsl/*
    ...
    retrieveKuopio
    cd $ROOT
    zip -D ${DEST}/router-waltti.zip router-waltti/*
else
    retrieveTampere
    retrieveHsl
    retrieveJyvaskyla
    ...
    retrieveKoontikanta
    cd $ROOT
    zip -D ${DEST}/router-finland.zip router-finland/*
fi
